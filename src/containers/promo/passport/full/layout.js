...
export default withNavigation(class PromoPassportLayout extends Component {
	constructor(props) {
		...
		this.inputs = {
			...
			mail: {
				ref: React.createRef(),
				offset: 1000*EStyleSheet.value("$scale"),
			},
		};

		this.required = ['name','father','family','birthday','seria','number','date','issuer','address','inn','mail'];
	...
	update = async (data) => {
		...
		if(this.state.inn.length		&& this.state.inn_error.length)			this.setState({inn_error:''});
	...
	check_completeness = async () => {
		...
		if(!state.mail.length){
			this.setState({mail_error:'Укажите почтовый ящик'});
